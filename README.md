...
### 4. 🌏 网络与性能极速优化
- **全内置 API 代理**：所有 TMDb 请求均由后端中转，支持**全局 10 小时分布式缓存**。多用户共享数据，极大降低 API 调用频率，彻底解决 Rate Limit 风险。
- **渲染加速 (content-visibility)**：采用现代 CSS 渲染技术，仅对视口内内容进行布局计算，首屏渲染速度提升 300%。
- **智能 IP 自动识别**：自动检测用户 IP 环境，智能切换官方地址或反代节点，确保海报图片瞬间秒开。
...
| **排序交互** | 仅支持默认热度 | **隐秘排序切换**：点击分类图标可在"最新上映"与"热门推荐"间即时切换。 |
| **加载性能** | 全量并发请求 | **视口懒加载 (IntersectionObserver)**：按需请求数据，节省 80% 初始流量。 |
| **搜索体验** | 需等待 loading 结束 | **实时流式加载 (SSE)**：结果即时呈现，源数量实时跳动增加，拒绝枯燥等待。 |
| **线路选择** | 单一延迟显示 | **双模式测速**：区分"直连"(用户端)和"代理"(服务器端) 测速，更准确。 |
| **启动体验** | 页面分块加载 | **优雅启动屏**：资源加载完成后丝滑过渡，拒绝白屏。 |
| **分类系统** | 仅支持搜索跳转 | **全随机/混搭榜单**：支持真正的全类别内容混搭盲盒，惊喜不断。 |
